...
# apply_mysql.sh
# 추천된 MySQL 서버 설정을 /etc/my.cnf.d에 배포합니다.
...
if [ ! -f "$RECOMM_FILE" ]; then
    ...
    log_error "추천 설정 파일이 없습니다" "apply_mysql" "$(json_kv file "$RECOMM_FILE")"
    exit 1
fi

mkdir -p "$TARGET_DIR" 2>/dev/null || {
    echo "[오류] ${TARGET_DIR} 디렉터리를 생성할 수 없습니다." >&2
    log_error "my.cnf.d 디렉터리를 생성하지 못했습니다" "apply_mysql" "$(json_kv dir "$TARGET_DIR")"
    exit 1
}

if [ -f "$TARGET_FILE" ]; then
    cp -a "$TARGET_FILE" "${TARGET_FILE}.backup.${TIMESTAMP}"
    log_info "기존 MySQL 튜닝 파일을 백업했습니다" "apply_mysql" "$(json_kv backup "${TARGET_FILE}.backup.${TIMESTAMP}")"
fi

cp -a "$RECOMM_FILE" "$TARGET_FILE" || {
    echo "[오류] ${TARGET_FILE} 파일을 생성하지 못했습니다." >&2
    log_error "추천 설정을 복사하는 데 실패했습니다" "apply_mysql"
    exit 1
}
chmod 644 "$TARGET_FILE" 2>/dev/null || true

# innodb_log_file_size 변경 시 기존 로그 파일 제거 필요 (MySQL 5.5/5.6 등 구버전)
if grep -q "innodb_log_file_size" "$TARGET_FILE" 2>/dev/null; then
    ...
    if [ -f "$DATADIR/ib_logfile0" ]; then
        log_info "InnoDB 로그 파일 크기 변경을 위해 기존 로그 파일을 삭제합니다" "apply_mysql" "$(json_kv datadir "$DATADIR")"
        # 서비스 중지
        if systemctl --quiet is-active mysqld.service 2>/dev/null; then
            systemctl stop mysqld >/dev/null 2>&1
        elif systemctl --quiet is-active mariadb.service 2>/dev/null; then
        ...
        log_info "InnoDB 로그 파일을 백업했습니다" "apply_mysql"
    fi
fi
...
log_info "MySQL 설정 적용을 완료했습니다" "apply_mysql" "$(json_kv target "$TARGET_FILE")"
