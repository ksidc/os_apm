...
run_background_step() {
    ...
    if [ -f "$script" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            log_info "단계가 성공적으로 완료되었습니다" "step" "$(json_two step "$step_number" pid "$pid")"
            log_section_end "성공"
        else
        ...
        runtime_log "단계 ${step_number} (${step_name}) = 건너뜀"
        log_warn "단계 스크립트를 찾을 수 없어 건너뜁니다" "step" "$(json_two step "$step_number" script "$script")"
        log_section_end "건너뜀"
    fi
}
...
run_backup_step() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "[경고] 백업 단계가 실패했습니다. 자세한 내용은 $DEBUG_LOG를 참고하세요." >&2
        log_section_end "실패"
    fi
}
...
gather_running_services() {
    ...
    if grep -q '^MARIADB_RUNNING=1' "$log_file" 2>/dev/null; then
        result+=("mariadb")
    fi
...
determine_target_services() {
    ...
    if profile_requires_db "$SERVICE_PROFILE"; then
        if printf '%s' " ${running_services[*]} " | grep -q ' mariadb '; then
            DB_SERVICE_TARGET="mariadb"
        elif printf '%s' " ${running_services[*]} " | grep -q ' mysql '; then
            DB_SERVICE_TARGET="mysql"
        else
            log_warn "DB 프로필이지만 실행 중인 DB 서비스가 없습니다" "detection" "$(json_kv profile "$SERVICE_PROFILE")"
        fi
    fi
    ...
    case "$SERVICE_PROFILE" in
        ...
        was_db|db)
            ...
            ;;
        *)
            log_warn "알 수 없는 서비스 프로필입니다" "detection" "$(json_kv profile "$SERVICE_PROFILE")"
            ;;
    esac
...
run_report_step() {
    ...
    if [ ! -f "$report_script" ]; then
        log_warn "보고서 생성 스크립트를 찾을 수 없습니다" "report" "$(json_kv path \"$report_script\")"
        return 0
    fi
    ...
    if bash "$report_script" >> "$DEBUG_LOG" 2>&1; then
        if [ -f "$REPORT_FILE" ]; then
            ...
            log_info "보고서 생성 완료" "report" "$(json_kv path \"$REPORT_FILE\")"
        else
            log_warn "보고서 스크립트가 성공했으나 파일을 찾을 수 없습니다" "report"
        fi
    else
