...
# 실행 중인 Apache/MySQL(MariaDB)의 버전을 확인합니다.
...
if [ -n "$apache_bin" ] && [ -x "$apache_bin" ]; then
    ...
    log_info "Apache 버전을 수집했습니다" "versions" "$(json_kv version "$apache_version")"
fi
...
if [ -n "$mysql_bin" ] && [ -x "$mysql_bin" ]; then
    ...
    write_version "MYSQL_VERSION" "${mysql_version:-unknown}"
    log_info "MySQL 버전을 수집했습니다" "versions" "$(json_kv version "${mysql_version:-unknown}")"
fi

mariadb_bin=$(kv_get_value "$SERVICE_LOG_PATH" "MARIADB_BINARY")
if [ -n "$mariadb_bin" ] && [ -x "$mariadb_bin" ]; then
    mariadb_version=$("$mariadb_bin" --version 2>/dev/null | awk '{print $3}')
    write_version "MARIADB_VERSION" "${mariadb_version:-unknown}"
    log_info "MariaDB 버전을 수집했습니다" "versions" "$(json_kv version "${mariadb_version:-unknown}")"
fi
