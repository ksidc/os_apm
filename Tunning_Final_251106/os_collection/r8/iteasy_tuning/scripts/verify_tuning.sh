...
APACHE_INCLUDE="/etc/httpd/conf.d/zz-iteasy_tuning.conf"
MYSQL_INCLUDE="/etc/my.cnf.d/zz-iteasy_tuning.cnf"
...
if [ -d "$BACKUP_DIR_PATH" ]; then
    note_ok "백업 디렉터리를 찾았습니다: $BACKUP_DIR_PATH"
else
    note_warn "백업 디렉터리가 발견되지 않았습니다: $BACKUP_DIR_PATH"
fi
...
if [ -f "$MYSQL_INCLUDE" ]; then
    note_ok "MySQL/MariaDB 포함 파일을 확인했습니다: $MYSQL_INCLUDE"
else
    note_warn "MySQL/MariaDB 포함 파일이 없습니다: $MYSQL_INCLUDE"
fi
