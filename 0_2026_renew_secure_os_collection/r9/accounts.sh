...
#   변경점: PASS_MAX_DAYS 기본 90, PASS_MIN_DAYS 기본 1 (기존 0)
# ────────────────────────────────────────────────────────────
set_password_policy() {
    ...
    if [[ "$ans" =~ ^[Yy]$ ]]; then
        ...
        chage -M "$max_days" -m "$min_days" -W "$warn_days" root \
            || log_error "set_password_policy" "root 설정 실패"
        log_info "root 패스워드 정책 설정 완료"
...
# ────────────────────────────────────────────────────────────
# [2026 수정] U-03: faillock.conf — deny=10, unlock_time=120 명시
# ────────────────────────────────────────────────────────────
configure_pam_lockout() {
    ...
    # faillock.conf에 deny/unlock_time 설정
...
unlock_time = 120
EOF
    log_info "faillock.conf 설정 완료 (deny=10, unlock_time=120)"
...
# ────────────────────────────────────────────────────────────
# [2026 수정] U-06: su 제한 — authselect 이후에 su 권한 재설정
#   문제: authselect apply-changes가 su 권한을 리셋할 수 있음
#   해결: authselect 이후에 set_file_perms 재실행
# ────────────────────────────────────────────────────────────
configure_su_restriction() {
    ...
    # authselect 이후에 su 권한 강제 재설정 (U-06)
