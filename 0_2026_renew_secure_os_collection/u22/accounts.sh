...
# ────────────────────────────────────────────────────────────
# [2026 수정] U-02: 비밀번호 정책 — PASS_MIN_DAYS 기본 1, root에도 적용
# ────────────────────────────────────────────────────────────
configure_password_policy() {
  ...
  # root 계정에도 적용 (2026 요구)
  chage -M "$max_days" -m "$min_days" -W "$warn_days" root || log_warn "root chage 적용 실패"
  log_info "root 비밀번호 정책 적용"
...
# ────────────────────────────────────────────────────────────
# [2026 수정] U-03: PAM 잠금 — deny=10, unlock_time=120
# ────────────────────────────────────────────────────────────
configure_pam_lockout() {
  ...
  if [[ "$use_faillock" == true ]]; then
    local preauth_line="auth    required pam_faillock.so preauth silent deny=10 unlock_time=120"
    local authfail_line="auth    [default=die] pam_faillock.so authfail deny=10 unlock_time=120"
    local authsucc_line="auth    sufficient pam_faillock.so authsucc deny=10 unlock_time=120"
    ...
    log_info "pam_faillock 기반 로그인 실패 잠금 정책을 적용했습니다. (deny=10, unlock_time=120)"
  else
    local auth_line="auth    required pam_tally2.so deny=10 onerr=fail unlock_time=120 even_deny_root"
    ...
    log_warn "pam_faillock을 사용할 수 없어 pam_tally2 기반 잠금 정책을 적용했습니다. (deny=10, unlock_time=120)"
  fi
}
