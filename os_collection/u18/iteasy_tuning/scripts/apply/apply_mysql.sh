...
# apply_mysql.sh
# MySQL 설정 파일을 tmp_conf/mysql_config.conf로 교체
...
# MySQL conf 목록 (MYSQL_CONF_*, MYSQL_CONF 모두)
MYSQL_CONF_LIST=()
MYSQL_LABELS=()
...
if [ "${#MYSQL_CONF_LIST[@]}" -eq 0 ]; then
    echo "[ERROR] 적용 대상 MySQL conf 파일이 없습니다." >&2
    log_debug "적용 대상 MySQL conf 파일이 없습니다."
    exit 1
fi

# MySQL 바이너리 경로 가져오기 (문법 체크용)
MYSQL_BINARY=$(grep '^MYSQL_BINARY=' "$SERVICE_PATHS" | cut -d'=' -f2)
if [ -z "$MYSQL_BINARY" ] || [ ! -x "$MYSQL_BINARY" ]; then
    echo "[ERROR] MySQL 바이너리($MYSQL_BINARY)가 없거나 실행 가능하지 않습니다." >&2
    log_debug "MySQL 바이너리($MYSQL_BINARY)가 없거나 실행 가능하지 않습니다."
    exit 1
fi

# MySQL 서비스 이름 가져오기 (중지/시작용)
MYSQL_SERVICE=$(grep '^MYSQL_SERVICE=' "$SERVICE_PATHS" | cut -d'=' -f2)
if [ -z "$MYSQL_SERVICE" ]; then
    echo "[ERROR] MySQL 서비스 이름(MYSQL_SERVICE)이 정의되지 않았습니다." >&2
    log_debug "MySQL 서비스 이름이 정의되지 않았습니다."
    exit 1
fi
...
for idx in "${IDX_LIST[@]}"; do
    ...
    # diff 파일 저장
    DIFF_FILE="$LOG_DIR/diff_mysql_$((idx+1))_$TIMESTAMP.diff"
    diff -u "$CONF_FILE" "$APPLY_SRC" > "$DIFF_FILE" 2>> "$LOG_DIR/apply_mysql_error.log"
    if [ $? -eq 0 ] && [ -s "$DIFF_FILE" ]; then
        log_debug "diff 파일 생성: $DIFF_FILE"
        echo "[INFO] 변경 내역 저장: $DIFF_FILE"
    else
        log_debug "diff 생성 실패: $CONF_FILE → $APPLY_SRC, 오류 로그: $LOG_DIR/apply_mysql_error.log"
        echo "[WARN] diff 생성 실패, 로그 확인: $LOG_DIR/apply_mysql_error.log" >&2
    fi
    ...
    echo "[DRY-RUN] $MYSQL_BINARY --defaults-file=$CONF_FILE --verbose --help (문법 체크)"
    "$MYSQL_BINARY" --defaults-file="$CONF_FILE" --verbose --help > /dev/null 2> "$LOG_DIR/apply_mysql_error.log"
    if [ $? -eq 0 ]; then
        echo "[dry-run] 특이사항 없음."
        # 서비스 중지, 대기, 시작
        systemctl stop "$MYSQL_SERVICE" >/dev/null 2>> "$LOG_DIR/apply_mysql_error.log"
        sleep 3
        systemctl start "$MYSQL_SERVICE" >/dev/null 2>> "$LOG_DIR/apply_mysql_error.log"
        if [ $? -eq 0 ]; then
            log_debug "$LABEL: MySQL 재시작 성공"
            echo "[OK] $LABEL: 설정 적용 및 재시작 완료"
        else
            log_debug "오류: $LABEL MySQL 재시작 실패"
        ...
        cat "$LOG_DIR/apply_mysql_error.log"
        ERR_CONF_FILE="$ERR_CONF_DIR/${BASE_NAME}.err.$TIMESTAMP"
        cp -a "$CONF_FILE" "$ERR_CONF_FILE" 2>> "$LOG_DIR/apply_mysql_error.log"
