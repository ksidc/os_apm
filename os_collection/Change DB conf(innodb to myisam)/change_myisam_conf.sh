#!/usr/bin/env bash
# gen_mysql_conf_myisam_auto_nobak.sh
# MyISAM 중심 자동 튜닝 (메모리 기반 항목만 재계산, 코어 기반/환경 항목은 유지)
# - 기존 주석 '# MyISAM 최소화' 삭제
# - '# MyISAM 성능 최적화 (메모리 기반)' 삽입
# - innodb_flush_method 제거
# - 백업 파일 생성하지 않음
...
/innodb_flush_method/ { next }        # innodb_flush_method 제거
/^#/ {
  if ($0 ~ /MyISAM 최소화/) next;     # 기존 주석 삭제
  print; next
}
