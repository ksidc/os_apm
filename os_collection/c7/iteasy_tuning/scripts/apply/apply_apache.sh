...
if grep -q '^APACHE_MPM_CONF_' "$SERVICE_PATHS"; then
    for i in $(seq 1 "$MULTIPLE_APACHE_FOUND"); do
        ...
        if [ -n "$conf" ] && [ "$conf" != "NOT_FOUND" ] && [ -f "$conf" ] && [ "$running" = "1" ]; then
            ...
            if [[ "$binary" == *"/usr/local/apache2/"* ]]; then
                MPM_CONFIG_FILE+=("/usr/local/apache2/conf/httpd.conf")
            elif [[ "$binary" == *"/usr/local/apache/"* ]]; then
    ...
    if [ -n "$MPM_CONF" ] && [ "$MPM_CONF" != "NOT_FOUND" ] && [ -f "$MPM_CONF" ] && [ "$APACHE_RUNNING" = "1" ]; then
        ...
        if [[ "$MPM_BINARY" == *"/usr/local/apache2/"* ]]; then
            MPM_CONFIG_FILE+=("/usr/local/apache2/conf/httpd.conf")
        elif [[ "$MPM_BINARY" == *"/usr/local/apache/"* ]]; then
            MPM_CONFIG_FILE+=("/usr/local/apache/conf/httpd.conf")
        else
            MPM_CONFIG_FILE+=("/etc/httpd/conf/httpd.conf")
        fi
...
for idx in "${!MPM_CONF_LIST[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        # 서비스 재시작
        if [[ "$BINARY" == *"/usr/local/apache2/"* ]]; then
            # 컴파일 설치 Apache의 경우 (/usr/local/apache2)
            PID_FILE="/usr/local/apache2/logs/httpd.pid"
            echo "[INFO] 컴파일 설치 Apache 재시작 시도: $BINARY -k graceful -f $CONFIG_FILE"
            # 기존 프로세스 확인
            if [ -f "$PID_FILE" ]; then
                OLD_PID=$(cat "$PID_FILE")
                if [ -n "$OLD_PID" ] && ps -p "$OLD_PID" > /dev/null; then
                    echo "[INFO] 기존 Apache 프로세스(PID: $OLD_PID) 종료 시도"
                    kill -TERM "$OLD_PID" 2>> "$LOG_DIR/apply_apache_error.log"
                    sleep 2
                    if ps -p "$OLD_PID" > /dev/null; then
                        echo "[WARN] PID $OLD_PID 종료 실패, 강제 종료 시도"
                        kill -9 "$OLD_PID" 2>> "$LOG_DIR/apply_apache_error.log"
                        sleep 1
                    fi
                fi
            fi
            # 새 프로세스 시작
            "$BINARY" -k start -f "$CONFIG_FILE" >/dev/null 2>> "$LOG_DIR/apply_apache_error.log"
            if [ $? -eq 0 ]; then
                NEW_PID=$(cat "$PID_FILE" 2>/dev/null)
                if [ -n "$NEW_PID" ] && ps -p "$NEW_PID" > /dev/null; then
                    log_debug "$LABEL: Apache 재시작 성공 (PID: $NEW_PID)"
                    echo "[OK] $LABEL: 설정 적용 및 재시작 완료 (PID: $NEW_PID)"
                else
                    log_debug "오류: $LABEL Apache 재시작 후 PID 확인 실패"
                    echo "[ERROR] $LABEL: Apache 재시작 후 PID 확인 실패" >&2
                ...
                log_debug "오류: $LABEL Apache 재시작 실패"
                echo "[ERROR] $LABEL: Apache 재시작 실패, 롤백" >&2
                cp -a "$BACKUP_FILE" "$CONF_FILE"
                log_debug "복구: $CONF_FILE ← $BACKUP_FILE"
                echo "[복구] $CONF_FILE ← $BACKUP_FILE"
                continue
            ...
            # 컴파일 설치 Apache의 경우 (/usr/local/apache)
