...
configure_rsyslog() {
  ...
  if dpkg -s rsyslog >/dev/null 2>&1; then
    ...
    systemctl restart rsyslog || log_warn "rsyslog 재시작에 실패했습니다."
    log_info "rsyslog를 원격 서버($RSYSLOG_SERVER)로 전송하도록 설정했습니다."
  else
...
disable_rhosts_hosts_equiv() {
  ...
  log_info "rhosts 및 hosts.equiv 파일을 제거했습니다."
}

disable_finger() {
  log_info "disable_finger 시작"
  if dpkg -s finger >/dev/null 2>&1; then
    systemctl disable --now finger >/dev/null 2>&1 || log_warn "finger 비활성화에 실패했습니다."
...
disable_anonymous_ftp() {
  ...
  if dpkg -s vsftpd >/dev/null 2>&1; then
    ...
    systemctl restart vsftpd || log_warn "vsftpd 재시작에 실패했습니다."
    log_info "vsftpd의 익명 FTP 접속을 차단했습니다."
  else
...
disable_r_services() {
  ...
  for svc in rsh rlogin rexec; do
    if dpkg -s "$svc" >/dev/null 2>&1; then
      systemctl disable --now "$svc" >/dev/null 2>&1 || log_warn "$svc 비활성화에 실패했습니다."
...
disable_dos_services() {
  ...
  if [[ -f /etc/inetd.conf ]]; then
    ...
    systemctl restart openbsd-inetd >/dev/null 2>&1 || log_warn "openbsd-inetd 재시작에 실패했습니다."
  fi
...
remove_automountd() {
  ...
  if dpkg -s autofs >/dev/null 2>&1; then
    systemctl disable --now autofs >/dev/null 2>&1 || log_warn "autofs 비활성화에 실패했습니다."
...
configure_ftp_shell() {
  ...
  if getent passwd ftp >/dev/null; then
    ...
    log_info "ftp 계정의 로그인 쉘을 /bin/false로 변경했습니다."
  fi
}
