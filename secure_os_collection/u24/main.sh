  ...
  if [ -n "$existing" ]; then
    ...
    if [[ "$yn" =~ ^[Yy]$ ]]; then
      ...
      if id "$UserName" &>/dev/null; then
        ...
        # (D) 계정 생성 및 비밀번호 설정
        useradd -m -G adm,sudo -s /bin/bash "$UserName" || { log_error "create_user" "계정 $UserName 생성 실패"; return 1; }
        echo "$UserName:$UserPassword" | chpasswd || { log_error "create_user" "$UserName 비밀번호 설정 실패"; return 1; }
