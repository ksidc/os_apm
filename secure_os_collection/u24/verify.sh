#!/bin/bash
#
# Ubuntu 24.04 보안 스크립트 적용 검증 도우미
...
환경 변수:
  SSH_PORT, RSYSLOG_SERVER, BACKUP_DIR 등으로 기대 값을 재정의할 수 있습니다.
EOF
}
...
# common.sh 로드 시 로그 파일이 생성되지 않도록 비움
LOG_FILE=""
RESULT_FILE=""
...
check_history_timeout() {
  ...
  if ! grep -Eq 'HISTTIMEFORMAT=.*%Y-%m-%d %H:%M:%S' "$profile"; then
    CHECK_MESSAGE='/etc/profile 에 HISTTIMEFORMAT 미설정'
    return 1
  fi
  if grep -Eq '(^|\s)TMOUT=600\b' "$profile"; then
    return 0
  fi
  CHECK_MESSAGE='/etc/profile 에 TMOUT=600 미설정'
  return 1
}
...
check_sysctl() {
  ...
  local mismatched=()
  local kv key expected actual
  for kv in "${expectations[@]}"; do
    key="${kv%%=*}"
    expected="${kv#*=}"
    actual="$(sysctl -n "$key" 2>/dev/null || echo '')"
    if [[ "$actual" != "$expected" ]]; then
      mismatched+=("$key=$actual (기대 $expected)")
    fi
  done
  if ((${#mismatched[@]} == 0)); then
    return 0
  fi
  CHECK_MESSAGE="sysctl 불일치: ${mismatched[*]}"
  return 1
}
...
check_limits() {
  ...
  if grep -Eq '^\* hard nofile 61200' "$file" \
     && grep -Eq '^\* soft nofile 61200' "$file" \
     && grep -Eq '^\* hard nproc 61200' "$file" \
     && grep -Eq '^\* soft nproc 61200' "$file"; then
    return 0
  fi
  CHECK_MESSAGE='nofile/nproc 값이 기대와 다릅니다'
  return 1
}

check_pam_lockout() {
  if ! command_exists faillock; then
    CHECK_MESSAGE='faillock 명령이 없습니다'
    return 1
  fi
  ...
  if ((module_found == 0)); then
    CHECK_MESSAGE='pam_faillock.so 모듈을 찾지 못했습니다'
    return 1
  fi

  if ! grep -q 'pam_faillock.so preauth' /etc/pam.d/common-auth; then
    CHECK_MESSAGE='common-auth에 pam_faillock preauth 라인이 없습니다'
    return 1
  fi
  if ! grep -q 'pam_faillock.so authfail' /etc/pam.d/common-auth; then
    CHECK_MESSAGE='common-auth에 pam_faillock authfail 라인이 없습니다'
    return 1
  fi
  if ! grep -Eq '^\s*account\s+required\s+pam_faillock\.so' /etc/pam.d/common-account; then
    CHECK_MESSAGE='common-account에 pam_faillock account 라인이 없습니다'
    return 1
  fi

  local failcnt
  failcnt="$(faillock --user root 2>/dev/null | awk '/failcnt/ {print $2}' || echo '')"
  if [[ -n "$failcnt" && "$failcnt" -gt 0 ]]; then
    CHECK_MESSAGE="root failcnt=$failcnt"
    return 1
  fi
  return 0
}
...
check_root_ssh() {
  ...
  CHECK_MESSAGE='sshd_config 에 PermitRootLogin no 가 설정되지 않았습니다'
  return 1
}
...
check_ssh_port() {
  ...
  CHECK_MESSAGE="현재 SSH 포트 $current (기대 $SSH_PORT)"
  return 1
}
...
print_section "시스템"
run_check "명령 이력 시간/세션 타임아웃" check_history_timeout
...
run_check "PAM faillock 프로필 활성화" check_pam_lockout
...
run_check "레거시 서비스 비활성화" check_services_disabled
