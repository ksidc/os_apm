...
# CentOS 7 기본 미러 종료에 대응해 Vault 저장소를 설정하고 yum 캐시를 갱신한다.
configure_yum_repos() {
...
gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-7
EOF
    yum clean all && yum makecache || { echo "ERROR: Failed to refresh YUM cache" >&2; exit 1; }
}
...
install_packages() {
    ...
    for pkg in "${pkgs[@]}"; do
        if ! rpm -q "$pkg" &>/dev/null; then
            yum install -y "$pkg" || { echo "ERROR: $pkg 설치 실패" >&2; exit 1; }
        fi
    done
}
...
configure_ntp() {
    yum install -y chrony || { echo "ERROR: chrony 설치 실패" >&2; exit 1; }
    ...
    chronyc makestep || { echo "ERROR: 즉시 동기화 실패" >&2; exit 1; }
...
configure_rsyslog() {
    chown root:root /etc/rsyslog.conf \
        || { echo "ERROR: /etc/rsyslog.conf 소유자 설정 실패" >&2; exit 1; }
    chmod 640 /etc/rsyslog.conf \
        || { echo "ERROR: /etc/rsyslog.conf 권한 설정 실패" >&2; exit 1; }
...
disable_selinux() {
    setenforce 0 || { echo "ERROR: setenforce 실패" >&2; exit 1; }
...
fix_world_writable() {
    ...
    while IFS= read -r f; do
        chmod o-w "$f" || { echo "ERROR: $f 권한 변경 실패" >&2; exit 1; }
        cnt=$((cnt + 1))
