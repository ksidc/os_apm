#!/bin/bash
# firewalld.sh : Rocky 10 firewalld 설정
...
log_info "firewalld 방화벽 설정을 시작합니다."

systemctl stop iptables 2>/dev/null || true
systemctl disable iptables 2>/dev/null || true
systemctl mask iptables 2>/dev/null || true
log_info "iptables 비활성화(mask) 완료"

if ! rpm -q firewalld >/dev/null 2>&1; then
    dnf install -y firewalld || { log_error "firewalld" "firewalld 설치 실패"; exit 1; }
    log_info "firewalld 설치 완료"
fi

systemctl unmask firewalld >/dev/null 2>&1 || true
systemctl enable --now firewalld >/dev/null 2>&1 || { log_error "firewalld" "firewalld 데몬 구동 실패"; exit 1; }

log_info "firewalld 정책 등록 중..."
...
if firewall-cmd --reload; then
    log_info "firewalld 규칙 적용 완료"
else
    echo "❌ firewalld --reload 실패"
    exit 1
fi
