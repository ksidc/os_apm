...
# ──────────────────────────────────────────────────────────
try {
    $adminSID = (Get-CimInstance Win32_UserAccount | Where SID -like '*-500').SID
    $admin    = Get-LocalUser | Where SID -eq $adminSID
    if ($admin) {
        if ($admin.Name -ne $NewAdminName) {
            ...
            Need-Reboot   # 일부 서비스 재시작 필요
        } else {
            Write-Warn "W-01: 이미 $NewAdminName 으로 변경됨"
        }
    } else { Write-Warn "W-01: 로컬 Administrator 계정이 존재하지 않음" }
...
try {
    ...
    Write-OK "W-04/47: 계정 잠금 임계값·기간 설정 완료 ($($Lockout.Threshold)회 / $($Lockout.Duration)분)"
} catch { Write-Err "W-04/47: 실패 ? $_" }
...
try {
    $cap="경고: 무단 접속 금지"
    $txt="이 시스템은 허가받은 사용자만 접속할 수 있습니다.`n무단 접속 시 법적 처벌을 받을 수 있습니다. `n`nApply version 251203"
...
try {
    ...
    Write-OK "추가: 원격 데스크톱 포트를 $NewPort 로 변경함"
} catch {
    Write-Err "원격 포트 변경 실패 ? $_"
}
...
try {
    ...
    Write-OK "추가: netsh 방식으로 포트 $NewPort 허용 규칙 추가"
} catch {
    Write-Err "방화벽 규칙 추가 실패 ? $_"
}
